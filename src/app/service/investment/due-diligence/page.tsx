...
// Define the interface for Due Diligence service info
interface DueDiligenceServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const dueDiligenceServices = [
  {
    ...
    detailedContent: `• Comprehensive document collection and organization
• Corporate structure and capitalization table documentation
• Financial statements and tax compliance records
• Intellectual property portfolio documentation
• Material contracts and agreements compilation
• Employee and HR-related documentation
• Regulatory and compliance certificates`
  },
  {
    ...
    category: "Assessment",
    detailedContent: `• Comprehensive risk identification and analysis
...
• Recommendations for risk mitigation before investor review`
  },
  {
    id: 3,
    icon: Search,
    ...
    category: "Guidance",
    detailedContent: `• End-to-end due diligence process management
• Investor query response coordination
• Virtual data room setup and management
• Timeline and milestone tracking
• Communication strategy with potential investors
• Post-diligence follow-up and action items
• Preparation for final investment negotiations`
  }
];

const benefits = [
  'Comprehensive preparation for investor scrutiny',
  'Identification and resolution of potential issues early',
  'Streamlined process saving time and resources',
];

export default function DueDiligencePage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<DueDiligenceServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: DueDiligenceServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    dueDiligenceServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: DueDiligenceServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Due Diligence Support hero"
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes document preparation, risk assessment, and process guidance to ensure your due diligence process is thorough, efficient, and positions your startup for successful fundraising.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert due diligence support:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Due Diligence Support visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: DueDiligenceServiceInfo; 
  index: number; 
  onClick: (info: DueDiligenceServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Due Diligence Support
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our due diligence support ensures your fundraising process is smooth, efficient, and positions your startup for successful investment.
        </p>
        <Link
          href="/contact"
