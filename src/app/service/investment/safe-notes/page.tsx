...
import { Package, FileText, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for SAFE Notes service info
interface SafeNotesServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// SAFE Notes services data
const safeNotesServices = [
  {
    id: 1,
    icon: Package,
    ...
    detailedContent: `• Custom SAFE agreement drafting based on your fundraising needs
• Valuation cap and discount rate considerations
• Pre-money vs. post-money SAFE options
• Conversion triggers and mechanisms
• Pro-rata rights and investor protections
• Tailored terms for your specific industry`
  },
  {
    id: 2,
    icon: FileText,
    ...
    category: "Drafting",
    detailedContent: `• Convertible note agreement drafting with clear conversion terms
• Interest rate and maturity date considerations
• Discount rates and valuation caps
• Conversion upon qualified financing events
• Maturity conversion and repayment options
...
• Security and collateral arrangements`
  },
  {
    ...
    category: "Compliance",
    detailedContent: `• Compliance with Companies Act and securities regulations
• FEMA and RBI regulations for foreign investment
• Tax implications of SAFE and convertible note instruments
• SEBI regulations compliance for fundraising
...
const benefits = [
  'Flexible fundraising without immediate equity dilution',
  'Clear terms for future conversion to equity',
  'Legal protection for both founders and investors',
];

export default function SafeNotesPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<SafeNotesServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: SafeNotesServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    safeNotesServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: SafeNotesServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="SAFE & Convertible Notes hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    SAFE (Simple Agreement for Future Equity) and convertible notes offer flexible ways to raise funds without immediate equity dilution. At Leonard Corporate Solutions, we structure these agreements to help your startup raise capital efficiently in India and worldwide.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes SAFE structuring, convertible note drafting, and legal compliance to ensure your fundraising instruments are properly structured, legally compliant, and aligned with your business goals.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert SAFE and convertible note services:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="SAFE & Convertible Notes visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: SafeNotesServiceInfo; 
  index: number; 
  onClick: (info: SafeNotesServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our SAFE & Convertible Note Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of SAFE & Convertible Notes
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our SAFE and convertible note services ensure your fundraising is structured properly, legally compliant, and aligned with your business growth objectives.
        </p>
        <Link
          href="/contact"
