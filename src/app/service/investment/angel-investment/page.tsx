...
import { FileText, HandshakeIcon, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Angel Investment service info
interface AngelInvestmentServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Angel Investment services data
const angelInvestmentServices = [
  {
    id: 1,
    icon: FileText,
    ...
    category: "Drafting",
    detailedContent: `• Investor-friendly term sheet creation
• Clear equity allocation and valuation terms
• Investment amount and disbursement schedule
• Voting rights and board representation clauses
• Founder vesting schedules and lock-in periods
• Information rights and reporting requirements
• Customized clauses for startup specifics`
  },
  {
    id: 2,
    icon: HandshakeIcon,
    ...
    detailedContent: `• Comprehensive term sheet analysis
• Negotiation strategy development
• Founder protection mechanisms
• Investor rights balancing
• Valuation justification and negotiation
• Liquidation preference structuring
• Anti-dilution protection review
• Exit strategy alignment`
  },
  {
    ...
    category: "Compliance",
    detailedContent: `• Regulatory compliance with Companies Act
• SEBI regulations adherence for angel investments
...
const benefits = [
  'Secure funding while maintaining founder control and vision',
  'Establish clear expectations and protections for both founders and investors',
  'Ensure legal compliance to prevent future disputes and regulatory issues',
];

export default function AngelInvestmentPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<AngelInvestmentServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: AngelInvestmentServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    angelInvestmentServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: AngelInvestmentServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes agreement drafting, term negotiation, and compliance checks to ensure your agreements protect all parties and support business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert angel investment agreements:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: AngelInvestmentServiceInfo; 
  index: number; 
  onClick: (info: AngelInvestmentServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Angel Investment Agreements
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our angel investment agreement services ensure your startup secures funding while maintaining the right balance between founder control and investor protection.
        </p>
        <Link
          href="/contact"
