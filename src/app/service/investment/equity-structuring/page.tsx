...
import { PieChart, Users, Scale, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Equity Structuring service info
interface EquityServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Equity Structuring services data
const equityServices = [
  {
    ...
    category: "Founders",
    detailedContent: `• Founder equity allocation and vesting schedules
• Equity splits based on roles, contributions, and future commitments
• Cliff periods and vesting acceleration provisions
• Founder agreements and shareholder arrangements
• Buyback provisions and exit scenarios
• Tax implications of founder equity allocations`
  },
  {
    id: 2,
    icon: Scale,
    ...
    category: "Investors",
    detailedContent: `• Investor equity structuring for funding rounds
• Preferred vs. common stock considerations
• Liquidation preferences and anti-dilution protections
• Voting rights and board representation
• Dividend policies and redemption rights
...
• Term sheet negotiation and documentation`
  },
  {
    ...
    category: "Employees",
    detailedContent: `• Employee Stock Option Plan (ESOP) design and implementation
• ESOP pool sizing and allocation strategies
• Granting methodology and vesting schedules
• Exercise price determination and tax implications
• Option exercise procedures and expiration policies
• Secondary sale provisions and liquidity events
• Communication strategies for employee equity education`
  }
];

const benefits = [
  'Fair equity distribution that aligns interests of all stakeholders',
  'Attract and retain top talent with competitive equity packages',
  'Structured approach to equity that supports future fundraising',
];

export default function EquityStructuringPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<EquityServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: EquityServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: EquityServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
        ...
        >
          <div className="relative flex flex-col lg:flex-row items-center">
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes founder equity allocation, investor equity structuring, and employee ESOPs to ensure your equity distribution supports growth, attracts talent, and aligns with your business objectives.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert equity structuring services:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  src="https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
                  alt="Equity Structuring visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: EquityServiceInfo; 
  index: number; 
  onClick: (info: EquityServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our equity structuring services ensure fair distribution, legal compliance, and alignment with your business growth objectives.
        </p>
        <Link
          href="/contact"
