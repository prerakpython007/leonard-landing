...
import { Search, Scale, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Regulatory Compliance service info
interface RegulatoryComplianceServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Regulatory Compliance services data
const regulatoryComplianceServices = [
  {
    ...
    category: "Assessment",
    detailedContent: `• Comprehensive regulatory gap analysis
• Risk assessment and identification
• Industry-specific compliance review
• Documentation and process evaluation
...
• Priority-based action plan`
  },
  {
    ...
    category: "Strategy",
    detailedContent: `• Sector-specific regulatory mapping
• Customized compliance framework development
• Policy and procedure documentation
• Compliance training programs
• Implementation roadmap
• Key performance indicators (KPIs)
• Regular compliance monitoring system
• Update mechanisms for regulatory changes`
  },
  {
    id: 3,
    icon: Shield,
    ...
    category: "Support",
    detailedContent: `• Regulatory change monitoring and alerts
• Quarterly compliance reviews
• Compliance helpline for queries
...
export default function RegulatoryCompliancePage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<RegulatoryComplianceServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: RegulatoryComplianceServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    regulatoryComplianceServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: RegulatoryComplianceServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="Regulatory Compliance hero"
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes compliance audits, industry-specific strategies, and ongoing support to keep your business protected from regulatory risks.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert compliance guidance:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: RegulatoryComplianceServiceInfo; 
  index: number; 
  onClick: (info: RegulatoryComplianceServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our regulatory compliance services ensure your business operates within legal frameworks, minimizing risks and building trust with stakeholders.
        </p>
        <Link
          href="/contact"
