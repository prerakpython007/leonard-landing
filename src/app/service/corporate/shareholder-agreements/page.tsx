...
// Define the interface for Shareholder Agreement service info
interface ShareholderAgreementServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Shareholder Agreement services data
const shareholderAgreementServices = [
  {
    id: 1,
    icon: FileText,
    ...
    category: "Drafting",
    detailedContent: `• Comprehensive analysis of your business structure
• Tailored agreement drafting based on shareholder requirements
• Clear definition of rights, responsibilities, and obligations
• Custom clauses for unique business situations
...
• Dividend distribution policies`
  },
  {
    ...
    category: "Protection",
    detailedContent: `• Conflict resolution mechanisms
...
• Mediation and negotiation frameworks`
  },
  {
    ...
    category: "Compliance",
    detailedContent: `• Compliance with Companies Act and other relevant laws
• International legal considerations for cross-border operations
• Tax implications review
• Regulatory compliance assessment
• Regular updates for changing legislation
• Cross-border agreement expertise
• Local and international legal alignment`
  }
];
...
export default function ShareholderAgreementsPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<ShareholderAgreementServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ShareholderAgreementServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    shareholderAgreementServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ShareholderAgreementServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Shareholder Agreements hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Clear shareholder agreements are vital for aligning interests and preventing disputes. At Leonard Corporate Solutions, we draft and review agreements to ensure clarity, compliance, and protection for your business in India and beyond.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes custom drafting, dispute prevention mechanisms, and legal review to ensure your agreements protect all parties and support business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert shareholder agreements:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Shareholder Agreements visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ShareholderAgreementServiceInfo; 
  index: number; 
  onClick: (info: ShareholderAgreementServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our shareholder agreement services ensure your business relationships are clearly defined, legally protected, and aligned for long-term success.
        </p>
        <Link
          href="/contact"
