...
import { FileText, Search, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Contract Drafting service info
interface ContractDraftingServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Contract Drafting services data
const contractDraftingServices = [
  {
    ...
    category: "Drafting",
    detailedContent: `• Non-Disclosure Agreements (NDAs) tailored to your needs
• Service agreements with clear terms and conditions
...
• Custom contracts for specific business needs`
  },
  {
    id: 2,
    icon: Search,
...
• Assessment of fairness and balance in terms
• Recommendations for negotiation points`
  },
  {
    ...
    category: "Support",
    detailedContent: `• Strategic advice for contract negotiations
• Representation of your interests in discussions
• Alternative clause suggestions
• Risk mitigation strategies
• Clear communication of your requirements
• Documentation of agreed terms
• Follow-up on implementation of negotiated terms`
  }
];
...
export default function ContractDraftingPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<ContractDraftingServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ContractDraftingServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ContractDraftingServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes custom contract drafting, thorough review of existing agreements, and negotiation support to ensure your business is protected in all contractual relationships.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert contract services:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ContractDraftingServiceInfo; 
  index: number; 
  onClick: (info: ContractDraftingServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our contract drafting services ensure your business is protected with clear, enforceable agreements that minimize risks and prevent disputes.
        </p>
        <Link
          href="/contact"
