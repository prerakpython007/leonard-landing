...
import { Building, FileText, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Company Formation service info
interface CompanyFormationServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Company Formation services data
const companyFormationServices = [
  {
    id: 1,
    icon: Building,
    title: 'Structure Selection',
    description: 'We help you choose the best business structure for your goals.',
    image: "https://images.unsplash.com/photo-1560472355-536de3962603?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Consultation",
    detailedContent: `• Comprehensive analysis of your business needs
• Guidance on Proprietorship, Partnership, OPC, LLP, or Pvt Ltd structures
• Assessment of tax implications for each structure
• Advice on liability protection options
...
• Industry-specific recommendations`
  },
  {
    ...
    detailedContent: `• Preparation of all required documents
...
• Certificate of Incorporation acquisition`
  },
  {
    ...
    category: "Compliance",
    detailedContent: `• GST registration and compliance
...
export default function CompanyFormationPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<CompanyFormationServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: CompanyFormationServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    companyFormationServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: CompanyFormationServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes structure selection, incorporation process management, and compliance setup to ensure your business is built on a solid foundation.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert company formation:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: CompanyFormationServiceInfo; 
  index: number; 
  onClick: (info: CompanyFormationServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our company formation services ensure your business is built on a solid legal foundation with the right structure and compliance setup for long-term success.
        </p>
        <Link
          href="/contact"
