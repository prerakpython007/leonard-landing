...
import { Globe, Shield, Target, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Market Monitoring service info
interface MarketMonitoringServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Market Monitoring services data
const marketMonitoringServices = [
  {
    id: 1,
    icon: Globe,
    ...
    category: "Online",
    detailedContent: `• Comprehensive monitoring of e-commerce platforms
• Detection of counterfeit listings across marketplaces
• Monitoring of social media platforms for IP violations
• Regular reporting of suspicious activities
• Advanced search algorithms to identify fakes
• Customized monitoring based on your product portfolio`
  },
  {
    id: 2,
    icon: Target,
    ...
    image: "https://images.unsplash.com/photo-1528747008803-f9f5cc8f1a64?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Physical",
    detailedContent: `• On-ground investigation in key markets
• Identification of counterfeit distribution networks
• Mystery shopping to obtain counterfeit samples
• Geographical mapping of counterfeit hotspots
• Coordination with local authorities for raids
• Evidence collection for legal proceedings`
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1579389083078-4e7018379f7e?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Enforcement",
    detailedContent: `• Formal takedown requests to platform administrators
• Follow-up on takedown requests until completion
• Counter-notice response strategies
• Platform policy compliance guidance
• Bulk listing removal for large-scale infringements
• Documentation and reporting of all takedown actions`
  }
];
...
export default function MarketMonitoringPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<MarketMonitoringServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: MarketMonitoringServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    marketMonitoringServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: MarketMonitoringServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1610319263685-dffecc4d4a15?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8bWFya2V0JTIwbW9uaXRvcmluZyUyMGluJTIwbGVnYWwlMjBzZWN0b3J8ZW58MHx8MHx8fDA%3D"
          alt="Market Monitoring hero"
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Detect Counterfeiting with Comprehensive Market Monitoring
        </p>
      </motion.div>
    </section>
  );
        ...
        >
          <div className="relative flex flex-col lg:flex-row items-center">
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes online surveillance, physical market checks, and takedown services to ensure your brand is protected from counterfeit threats.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert market monitoring:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  src="https://images.unsplash.com/photo-1560472355-536de3962603?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
                  alt="Market Monitoring visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: MarketMonitoringServiceInfo; 
  index: number; 
  onClick: (info: MarketMonitoringServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our Market Monitoring Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our market monitoring services ensure your brand is protected from counterfeit threats through comprehensive surveillance and strategic enforcement.
        </p>
        <Link
          href="/contact"
