...
import { Mail, Gavel, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Legal Action service info
interface LegalServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const legalServices = [
  {
    ...
    category: "Notices",
    detailedContent: `• Professional drafting of cease and desist notices
• Strategic delivery to maximize impact
• Follow-up procedures for non-compliance
• Multi-jurisdictional notice coordination
• Customized approach based on infringer type
• Documentation for potential legal proceedings`
  },
  {
    ...
    category: "Litigation",
    detailedContent: `• Comprehensive case evaluation and strategy development
• Evidence collection and documentation
• Court representation by experienced IP attorneys
• Coordination with local counsel in different jurisdictions
• Management of all legal proceedings
• Regular updates and consultation throughout the process
• Settlement negotiation when appropriate`
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1604871000636-074fa5117945?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Remedies",
    detailedContent: `• Filing for temporary and permanent injunctions
• Calculation of damages from counterfeit activities
• Pursuit of statutory and actual damages
• Asset freezing requests against infringers
• Coordination with law enforcement for enforcement
• Monitoring compliance with court orders
• International injunction coordination`
  }
];

const benefits = [
  'Deter future counterfeiting through strong legal precedents',
  'Recover losses caused by counterfeit sales with comprehensive damage claims',
  'Protect your brand\'s integrity and market position with decisive legal action',
];

export default function LegalActionSupportPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<LegalServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: LegalServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: LegalServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1457369804613-52c61a468e7d?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="Legal Action Support hero"
        ...
        >
          <div className="relative flex flex-col lg:flex-row items-center">
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our legal action services include cease and desist notices, litigation support, and pursuing injunctions and damages to ensure your brand is protected through the legal system.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert legal action support:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  src="https://images.unsplash.com/photo-1589391886645-d51941baf7fb?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
                  alt="Legal Action Support visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: LegalServiceInfo; 
  index: number; 
  onClick: (info: LegalServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our legal action support services ensure your brand is protected through comprehensive legal strategies and enforcement actions.
        </p>
        <Link
          href="/contact"
