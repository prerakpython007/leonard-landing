...
import { BrainCog, LineChart, Microscope, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Counterfeit Detection service info
interface DetectionServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const detectionServices = [
  {
    ...
    category: "Analysis",
    detailedContent: `• Advanced AI algorithms for counterfeit pattern recognition
• Machine learning models trained on authentic vs. counterfeit product data
• Image recognition technology to identify visual inconsistencies
• Data analytics to detect anomalies in product distribution
• Real-time monitoring of online marketplaces
• Automated alerts for suspicious product listings`
  },
  {
    id: 2,
    icon: LineChart,
    ...
    category: "Tracking",
    detailedContent: `• Comprehensive supply chain mapping and analysis
• Blockchain technology for product authentication
• Serial number tracking and verification systems
• Distribution channel monitoring
• Geographic mapping of counterfeit hotspots
• Investigation of manufacturing sources
• Collaboration with logistics partners to identify leaks`
  },
  {
    id: 3,
    icon: Microscope,
    ...
    detailedContent: `• Physical product examination and forensic analysis
• Material composition testing
• Packaging and labeling verification
• Hologram and security feature authentication
• Chemical analysis of products
• Microscopic examination for manufacturing inconsistencies
• Certification of authenticity programs
• Expert witness services for legal proceedings`
  }
];

const benefits = [
  'Protect your brand\'s reputation and customer trust with advanced counterfeit detection',
  'Reduce revenue loss caused by counterfeit sales through proactive monitoring',
  'Gain actionable insights to take down counterfeiters and protect your market share',
];

export default function CounterfeitDetectionPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<DetectionServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: DetectionServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: DetectionServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1505664194779-8beaceb93744?q=80&w=800&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="Counterfeit Detection hero"
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Advanced Solutions for Identifying and Combating Counterfeit Products
        </p>
      </motion.div>
    </section>
  );
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    At Leonard Corporate Solutions, we specialize in advanced counterfeit detection, helping you protect your brand from fraudulent products. Whether it's identifying fake goods in the market or tracking down their sources, our comprehensive approach ensures your intellectual property and revenue are protected.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our counterfeit detection services include technology-driven analysis, supply chain tracking, and product authentication to ensure your brand's integrity is maintained across all markets and distribution channels.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert counterfeit detection:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Counterfeit Detection visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: DetectionServiceInfo; 
  index: number; 
  onClick: (info: DetectionServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            // style={{ willChange: 'transform, opacity' }}
          >
            Our Counterfeit Detection Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our counterfeit detection services ensure your brand is protected from fraudulent products with advanced technology and expert analysis.
        </p>
        <Link
          href="/contact"
