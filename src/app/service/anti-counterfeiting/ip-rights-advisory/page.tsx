...
import { Search, FileCheck, Target, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for IP Advisory service info
interface IPAdvisoryServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// IP Advisory services data
const ipAdvisoryServices = [
  {
    id: 1,
    icon: Search,
    title: 'IP Audit',
    ...
    category: "Assessment",
    detailedContent: `• Comprehensive analysis of your IP portfolio
• Identification of protection gaps and vulnerabilities
• Evaluation of IP registration status across jurisdictions
• Assessment of IP commercialization opportunities
...
• Documentation and application preparation`
  },
  {
    ...
    detailedContent: `• IP enforcement strategy development
• Cease and desist notice drafting
• Customs recordation assistance
• Online platform takedown strategies
• Litigation support and coordination
• Anti-counterfeiting program design
...
export default function IPRightsAdvisoryPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<IPAdvisoryServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: IPAdvisoryServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    ipAdvisoryServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: IPAdvisoryServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1637763723578-79a4ca9225f7?q=80&w=1742&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="IP Rights Advisory hero"
        ...
        >
          <div className="relative flex flex-col lg:flex-row items-center">
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes IP audits, registration support, and enforcement strategies to ensure your intellectual property is fully protected.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert IP advisory:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  src="https://images.unsplash.com/photo-1459499362902-55a20553e082?q=80&w=1738&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
                  alt="IP Rights Advisory visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: IPAdvisoryServiceInfo; 
  index: number; 
  onClick: (info: IPAdvisoryServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of IP Rights Advisory
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our IP rights advisory services ensure your intellectual property is fully protected through comprehensive planning and strategic enforcement.
        </p>
        <Link
          href="/contact"
