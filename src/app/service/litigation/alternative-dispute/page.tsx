...
import { Scale, Handshake, MessageSquare, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for ADR service info
interface ADRServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const adrServices = [
  {
    ...
    category: "Mediation",
    detailedContent: `• Facilitation of negotiations between disputing parties
• Development of mutually beneficial agreements
• Confidential and impartial mediation process
• Preservation of business relationships
• Cost-effective alternative to litigation
• Customized mediation strategies for each case`
  },
  {
    id: 2,
    icon: Scale,
    ...
    detailedContent: `• Representation in arbitration proceedings
• Preparation of arbitration documents and evidence
• Expert advocacy during arbitration hearings
• Enforcement of arbitration awards
• Domestic and international arbitration expertise
• Binding resolution of disputes
• Cost and time efficient compared to court litigation`
  },
  {
    id: 3,
    icon: MessageSquare,
    ...
    category: "Guidance",
    detailedContent: `• Comprehensive guidance through ADR processes
• Selection of appropriate dispute resolution method
• Strategic planning for optimal outcomes
• Documentation and procedural assistance
...
const benefits = [
  ...
  'Maintain business relationships through mediation',
  'Achieve binding resolutions with arbitration',
];

export default function AlternativeDisputePage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<ADRServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ADRServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ADRServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1589652717521-10c0d092dea9?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="Alternative Dispute Resolution hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Alternative Dispute Resolution (ADR) offers a faster, less adversarial way to settle disputes. At Leonard Corporate Solutions, we provide expert mediation and arbitration services to resolve conflicts efficiently in India and internationally.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes mediation, arbitration, and process guidance to ensure your disputes are resolved efficiently, confidentially, and with optimal outcomes for all parties involved.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert ADR services:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Alternative Dispute Resolution visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ADRServiceInfo; 
  index: number; 
  onClick: (info: ADRServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our ADR services ensure your disputes are resolved efficiently, confidentially, and with optimal outcomes for all parties involved.
        </p>
        <Link
          href="/contact"
