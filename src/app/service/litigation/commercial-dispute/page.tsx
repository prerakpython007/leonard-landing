...
import { Briefcase, Scale, Search, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Commercial Dispute service info
interface CommercialDisputeServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Commercial Dispute services data
const commercialDisputeServices = [
  {
    id: 1,
    icon: Search,
    ...
    category: "Analysis",
    detailedContent: `• Comprehensive assessment of dispute nature and impact
• Evaluation of legal merits and potential outcomes
• Analysis of financial implications and risks
• Identification of key stakeholders and their interests
• Documentation review and evidence assessment
• Preliminary strategy recommendations`
  },
  {
    ...
    category: "Planning",
    detailedContent: `• Customized dispute resolution strategy development
• Negotiation planning and preparation
• Alternative dispute resolution (ADR) options evaluation
• Litigation strategy when necessary
• Timeline and resource planning
• Cost-benefit analysis of different approaches
• Communication strategy with all parties`
  },
  {
    id: 3,
    icon: Scale,
    ...
    category: "Execution",
    detailedContent: `• Full support through negotiation processes
• Mediation and arbitration representation
• Litigation management and coordination
• Document preparation and filing assistance
• Settlement agreement drafting and review
• Enforcement of resolutions and judgments
• Post-resolution compliance monitoring
• Ongoing advisory for dispute prevention`
  }
];

const benefits = [
  'Protect your business with strategic dispute resolution',
  'Minimize costs through effective planning',
  'Achieve favorable outcomes with expert guidance',
];

export default function CommercialDisputePage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<CommercialDisputeServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: CommercialDisputeServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    commercialDisputeServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: CommercialDisputeServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Commercial Dispute Strategy hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Commercial disputes can disrupt your business if not managed strategically. At Leonard Corporate Solutions, we develop tailored plans to resolve disputes, protecting your interests in India and globally.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes dispute analysis, strategic planning, and execution support to ensure your commercial disputes are resolved efficiently with minimal business disruption.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert commercial dispute resolution:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Commercial Dispute Strategy visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: CommercialDisputeServiceInfo; 
  index: number; 
  onClick: (info: CommercialDisputeServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Strategic Dispute Resolution
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our commercial dispute services ensure your business disputes are resolved strategically, protecting your interests and minimizing disruption to your operations.
        </p>
        <Link
          href="/contact"
