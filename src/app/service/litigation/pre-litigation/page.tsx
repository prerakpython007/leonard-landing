...
import { FileText, MessageSquare, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Pre-Litigation service info
interface PreLitigationServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Pre-Litigation services data
const preLitigationServices = [
  {
    id: 1,
    icon: FileText,
    title: 'Notice Drafting',
    description: 'We create clear, legally sound notices to address disputes.',
    image: "https://images.unsplash.com/photo-1589652717521-10c0d092dea9?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Drafting",
    detailedContent: `• Professional legal notice drafting for various disputes
• Customized notices based on specific case requirements
• Clear communication of grievances and demands
• Compliance with legal format and requirements
• Strategic language to maximize impact
• Multiple revisions until client satisfaction`
  },
  {
    id: 2,
    icon: MessageSquare,
    ...
    category: "Management",
    detailedContent: `• Comprehensive response analysis and strategy development
• Professional reply drafting to counter notices
• Negotiation support for amicable settlements
• Mediation between parties to avoid litigation
• Documentation of all communications
• Regular updates on response status and developments`
  },
  {
    id: 3,
    icon: Shield,
    ...
    category: "Prevention",
    detailedContent: `• Early intervention strategies to prevent disputes from escalating
• Alternative dispute resolution methods
• Settlement agreement drafting and negotiation
• Legal counseling on dispute avoidance
• Relationship preservation techniques
• Cost-benefit analysis of litigation vs. settlement
• Long-term conflict resolution planning`
  }
];

const benefits = [
  'Resolve disputes early to save time and costs',
  'Communicate effectively with legally sound notices',
  'Prevent escalation with strategic negotiations',
];

export default function PreLitigationPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<PreLitigationServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: PreLitigationServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    preLitigationServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: PreLitigationServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Pre-Litigation Notices hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Pre-litigation notices can resolve disputes before they reach court, saving time and costs. At Leonard Corporate Solutions, we draft and manage notices to settle conflicts early in India and beyond.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes notice drafting, response management, and escalation prevention to ensure your disputes are resolved efficiently without the need for lengthy court proceedings.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert pre-litigation services:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Pre-Litigation Notices visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: PreLitigationServiceInfo; 
  index: number; 
  onClick: (info: PreLitigationServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Pre-Litigation Notices
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our pre-litigation services ensure your disputes are resolved efficiently, saving you time, money, and the stress of lengthy court proceedings.
        </p>
        <Link
          href="/contact"
