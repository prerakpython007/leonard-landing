...
import { Gavel, FileText, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Courtroom Representation service info
interface CourtroomServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Courtroom Representation services data
const courtroomServices = [
  {
    id: 1,
    icon: Gavel,
    ...
    category: "Advocacy",
    detailedContent: `• Experienced courtroom representation by skilled attorneys
• Strategic case presentation and argumentation
• Cross-examination of witnesses and evidence
• Persuasive legal reasoning and rhetoric
• Courtroom etiquette and procedural compliance
• Real-time adaptation to courtroom dynamics`
  },
  {
    id: 2,
    icon: FileText,
    ...
    detailedContent: `• Comprehensive evidence gathering and analysis
• Legal research and precedent identification
• Witness preparation and testimony organization
...
• Timeline and chronology development`
  },
  {
    ...
    category: "Strategy",
    detailedContent: `• Customized defense strategies for your specific case
• Risk assessment and mitigation planning
• Settlement evaluation and negotiation support
• Alternative dispute resolution options
• Appellate strategy when necessary
• Crisis management and communication planning
• Long-term business protection approaches`
  }
];

const benefits = [
  'Defend your interests with skilled advocacy',
  'Build a strong case with thorough preparation',
  'Protect your business with strategic defenses',
];

export default function CourtroomRepresentationPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<CourtroomServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: CourtroomServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    courtroomServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: CourtroomServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Courtroom Representation hero"
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    When disputes escalate to litigation, you need skilled representation. At Leonard Corporate Solutions, we provide expert courtroom advocacy to protect your business interests in India and internationally.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes legal advocacy, case preparation, and strategic defense to ensure your business is properly represented and protected in legal proceedings.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert courtroom representation:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Courtroom Representation visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: CourtroomServiceInfo; 
  index: number; 
  onClick: (info: CourtroomServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our Courtroom Representation Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Courtroom Representation
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our courtroom representation services ensure your business is properly defended, legally protected, and strategically positioned in legal proceedings.
        </p>
        <Link
          href="/contact"
