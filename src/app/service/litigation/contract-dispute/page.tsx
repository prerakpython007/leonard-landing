...
import { Scale, Search, Handshake, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Contract Dispute service info
interface ContractDisputeServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Contract Dispute services data
const contractDisputeServices = [
  {
    id: 1,
    icon: Search,
    ...
    category: "Assessment",
    detailedContent: `• Comprehensive analysis of contract breaches and violations
• Impact assessment on business operations and finances
• Documentation review and evidence gathering
• Identification of contractual obligations and responsibilities
• Evaluation of potential remedies and compensation
• Expert opinion on breach severity and implications`
  },
  {
    id: 2,
    icon: Scale,
    ...
    category: "Legal",
    detailedContent: `• Strategic negotiation with opposing parties
• Mediation and alternative dispute resolution (ADR)
• Litigation preparation and court representation
• Drafting of legal notices and court documents
• Enforcement of contractual rights and remedies
• International dispute resolution for cross-border contracts
• Settlement agreement drafting and negotiation`
  },
  {
    id: 3,
    icon: Handshake,
    ...
    image: "https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?q=80&w=1738&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Resolution",
    detailedContent: `• Relationship preservation strategies during disputes
• Communication management between parties
• Post-resolution relationship rebuilding
• Contract amendment and modification services
• Preventive measures to avoid future disputes
• Ongoing compliance monitoring and support
• Training for internal teams on dispute prevention`
  }
];

const benefits = [
  'Address contract breaches with precision',
  'Minimize financial losses through expert intervention',
  'Preserve business relationships during disputes',
];

export default function ContractDisputePage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<ContractDisputeServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ContractDisputeServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    contractDisputeServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ContractDisputeServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Contract Dispute Resolution hero"
        ...
        >
          <div className="relative flex flex-col lg:flex-row items-center">
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Contract breaches can disrupt your business and lead to losses. At Leonard Corporate Solutions, we provide precise legal interventions to resolve disputes, protecting your interests in India and internationally.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes breach assessment, legal action, and resolution support to ensure your contractual rights are protected and disputes are resolved efficiently.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert contract dispute resolution:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  src="https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?q=80&w=1738&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
                  alt="Contract Dispute Resolution visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ContractDisputeServiceInfo; 
  index: number; 
  onClick: (info: ContractDisputeServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our contract dispute resolution services ensure your contractual rights are protected, disputes are resolved efficiently, and your business relationships are preserved.
        </p>
        <Link
          href="/contact"
