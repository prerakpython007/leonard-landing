...
import { FileText, Shield, Settings, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Policy Development service info
interface PolicyDevelopmentServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Policy Development services data
const policyDevelopmentServices = [
  {
    id: 1,
    icon: FileText,
    title: 'Policy Drafting',
    description: 'We create policies on HR, ethics, compliance, and more.',
...
• Industry-specific policy development`
  },
  {
    ...
    description: 'We ensure policies meet local and international regulations.',
    ...
    category: "Compliance",
    detailedContent: `• Local regulatory compliance assessment
• International compliance frameworks
• Industry-specific regulation alignment
• Regular policy updates for changing legislation
...
• Regulatory change monitoring`
  },
  {
    ...
    description: 'We guide you in rolling out policies across your organization.',
    ...
    category: "Support",
    detailedContent: `• Policy rollout strategy development
...
const benefits = [
  'Promote consistency and transparency in business operations',
  'Ensure compliance with legal and regulatory standards',
  'Reduce risks by setting clear guidelines for employees',
];

export default function PolicyDevelopmentPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<PolicyDevelopmentServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: PolicyDevelopmentServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    policyDevelopmentServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: PolicyDevelopmentServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Govern Effectively with Compliant Corporate Policies
        </p>
      </motion.div>
    </section>
  );
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Well-defined corporate policies ensure your business operates smoothly and compliantly. At Leonard Corporate Solutions, we create tailored policies to govern your operations, promoting transparency and accountability in India and beyond.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes policy drafting, compliance alignment, and implementation support to ensure your policies protect your business and support operational excellence.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert policy development:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: PolicyDevelopmentServiceInfo; 
  index: number; 
  onClick: (info: PolicyDevelopmentServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Policy Development
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our policy development services ensure your business operates with clear guidelines, legal compliance, and operational excellence.
        </p>
        <Link
          href="/contact"
