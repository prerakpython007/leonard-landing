...
import { Scale, Search, FileCheck, AlertTriangle, FileText, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Compliance Audit service info
interface ComplianceAuditServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Compliance Audit services data
const complianceAuditServices = [
  {
    id: 1,
    icon: Search,
    ...
    category: "Assessment",
    detailedContent: `• Thorough evaluation of business operations
• Assessment against all applicable regulations
...
• Operational process evaluation`
  },
  {
    ...
    category: "Analysis",
    detailedContent: `• Identification of compliance gaps
...
• Benchmarking against industry standards`
  },
  {
    ...
    detailedContent: `• Comprehensive audit reports
• Clear recommendations for compliance
...
export default function ComplianceAuditsPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<ComplianceAuditServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ComplianceAuditServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    complianceAuditServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ComplianceAuditServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes detailed assessment, gap analysis, and comprehensive reporting to ensure your business remains compliant with evolving regulations.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert compliance audits:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ComplianceAuditServiceInfo; 
  index: number; 
  onClick: (info: ComplianceAuditServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our compliance audit services ensure your business meets all regulatory requirements, minimizing risks and building stakeholder confidence.
        </p>
        <Link
          href="/contact"
