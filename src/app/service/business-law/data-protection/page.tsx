...
import { Lock, FileText, Search, Settings, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Data Protection service info
interface DataProtectionServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const dataProtectionServices = [
  {
    ...
    category: "Assessment",
    detailedContent: `• Comprehensive GDPR compliance assessment
• DPDP Act (India) compliance evaluation
• International data protection law review
• Data processing activity mapping
• Gap analysis and risk assessment
• Vendor and third-party compliance checks
• Data transfer mechanism evaluation`
  },
  {
    id: 2,
    icon: FileText,
    ...
    detailedContent: `• Privacy policy creation and customization
...
• Data protection impact assessments (DPIAs)`
  },
  {
    ...
    category: "Education",
    detailedContent: `• Employee data protection training programs
• Role-specific privacy awareness sessions
• Data Protection Officer (DPO) support
• Ongoing compliance monitoring
• Regulatory update briefings
• Incident response training
• Customized workshops for your industry
...
export default function DataProtectionPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<DataProtectionServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: DataProtectionServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: DataProtectionServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
  ...
  return (
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1621905252507-b35492cc74b4?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="Data Protection hero"
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes compliance audits, policy development, and training to ensure your business meets all data protection requirements and builds trust with your customers.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert data protection compliance:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: DataProtectionServiceInfo; 
  index: number; 
  onClick: (info: DataProtectionServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our data protection services ensure your business complies with regulations, protects customer data, and builds trust in today's digital economy.
        </p>
        <Link
          href="/contact"
