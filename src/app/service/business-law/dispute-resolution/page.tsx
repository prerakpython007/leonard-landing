...
import { Gavel, Handshake, Scale, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Dispute Resolution service info
interface DisputeResolutionServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Dispute Resolution services data
const disputeResolutionServices = [
  {
    id: 1,
    icon: Handshake,
    title: 'Negotiation & Mediation',
    description: 'We explore amicable solutions to avoid litigation.',
    ...
    category: "Resolution",
    detailedContent: `• Expert negotiation strategies for business disputes
• Professional mediation services to find common ground
• Confidential dispute resolution processes
• Preservation of business relationships
• Cost-effective alternatives to litigation
• Customized approaches for different types of disputes`
  },
  {
    id: 2,
    icon: Scale,
    title: 'Arbitration Support',
    description: 'We represent you in arbitration proceedings for faster resolutions.',
    ...
    category: "Support",
    detailedContent: `• Representation in domestic and international arbitration
• Preparation of arbitration agreements and clauses
• Case strategy development and evidence preparation
...
• Alternative dispute resolution (ADR) guidance`
  },
  {
    ...
    description: 'We provide legal representation in court if disputes escalate.',
    ...
    category: "Legal",
    detailedContent: `• Comprehensive litigation strategy development
• Court representation for business disputes
• Document preparation and legal filings
• Case management and progress tracking
• Settlement negotiation and facilitation
• Appeals process guidance and representation`
  }
];

const benefits = [
  'Resolve conflicts quickly to minimize operational disruptions',
  'Protect your business\'s reputation and relationships',
  'Save time and costs with efficient resolution strategies',
];

export default function DisputeResolutionPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<DisputeResolutionServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: DisputeResolutionServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: DisputeResolutionServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Navigate Business Disputes with Expert Support
        </p>
      </motion.div>
    </section>
  );
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Business disputes can disrupt operations and harm your reputation. At Leonard Corporate Solutions, we provide expert dispute resolution support, offering legal guidance and representation to resolve conflicts efficiently in India and internationally.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes negotiation, mediation, arbitration support, and litigation assistance to protect your business interests and maintain positive relationships.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert dispute resolution support:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: DisputeResolutionServiceInfo; 
  index: number; 
  onClick: (info: DisputeResolutionServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our dispute resolution services ensure your business conflicts are resolved efficiently, protecting your operations and preserving valuable relationships.
        </p>
        <Link
          href="/contact"
