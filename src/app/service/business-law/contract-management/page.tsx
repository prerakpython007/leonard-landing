...
// Define the interface for Contract Management service info
interface ContractManagementServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Contract Management services data
const contractManagementServices = [
  {
    id: 1,
    icon: FileText,
    title: 'Contract Drafting',
    description: 'We create clear, enforceable contracts tailored to your specific business needs.',
    ...
    category: "Drafting",
    detailedContent: `• Custom contract creation for your specific business requirements
• Clear and enforceable terms and conditions
• Industry-specific contract provisions
• Risk mitigation through precise language
...
• Custom clauses for unique business situations`
  },
  {
    ...
    category: "Analysis",
    detailedContent: `• Comprehensive analysis of existing contracts
• Identification of potential risks and liabilities
• Recommendations for improvements and modifications
• Compliance verification with current laws
• Assessment of unfavorable terms and conditions
• Gap analysis for missing essential clauses
• Suggestions for negotiation points`
  },
  {
    ...
    category: "Compliance",
    detailedContent: `• Systematic tracking of contract timelines and deadlines
• Automated renewal notifications and management
• Monitoring of contractual obligations and commitments
• Centralized contract repository for easy access
• Regular compliance audits and reporting
• Performance tracking against contract terms
• Amendment management for contract changes`
  }
];
...
export default function ContractManagementPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<ContractManagementServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: ContractManagementServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    contractManagementServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: ContractManagementServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes custom drafting, thorough review processes, and systematic management to ensure your contracts protect your interests and support business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert contract management:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Contract Management visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: ContractManagementServiceInfo; 
  index: number; 
  onClick: (info: ContractManagementServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our contract management services ensure your business agreements are clearly defined, legally protected, and aligned for secure business transactions.
        </p>
        <Link
          href="/contact"
