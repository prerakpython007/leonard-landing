...
import { Users, Scale, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Founder Agreement service info
interface FounderAgreementServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Founder Agreement services data
const founderAgreementServices = [
  {
    id: 1,
    icon: Users,
    ...
    category: "Structure",
    detailedContent: `• Clear definition of each founder's roles and responsibilities
...
• Conflict resolution mechanisms for role disputes`
  },
  {
    id: 2,
    icon: Scale,
    ...
    category: "Finance",
    detailedContent: `• Equity allocation based on contributions and roles
• Vesting schedules with cliffs and acceleration provisions
• Equity for future employees and advisors
• Dilution protection mechanisms
• Buyback provisions and transfer restrictions
• Valuation methodologies for equity contributions
• Tax implications of equity structures`
  },
  {
    ...
    category: "Protection",
    detailedContent: `• Conflict resolution mechanisms and escalation processes
• Deadlock breaking procedures for decision-making
• Founder exit strategies and buyout provisions
• Non-compete and confidentiality agreements
• Intellectual property protection and assignment
• Dispute mediation and arbitration processes
...
export default function FounderAgreementsPage() {
  ...
  const [selectedInfo, setSelectedInfo] = useState<FounderAgreementServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: FounderAgreementServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    founderAgreementServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: FounderAgreementServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes role definition, equity structuring, and dispute resolution mechanisms to ensure your agreements protect all founders and support business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert founder agreements:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: FounderAgreementServiceInfo; 
  index: number; 
  onClick: (info: FounderAgreementServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our Founder Agreement Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm">
        <h2 className="text-[#eeeeee] text-2xl md:text-3xl lg:text-4xl font-extrabold text-center mb-6 md:mb-8"
          ...
          Benefits of Founder Agreements
        </h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our founder agreement services ensure your startup relationships are clearly defined, legally protected, and aligned for long-term success.
        </p>
        <Link
          href="/contact"
