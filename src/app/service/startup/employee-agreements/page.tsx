...
import { UserCheck, FileText, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Employee Agreement service info
interface EmployeeAgreementServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Employee Agreement services data
const employeeAgreementServices = [
  {
    id: 1,
    icon: UserCheck,
    ...
    category: "Employment",
    detailedContent: `• Comprehensive employment contracts tailored to your industry
• Clear definition of roles, responsibilities, and reporting structure
• Compensation structure including salary, bonuses, and benefits
• Confidentiality and non-disclosure agreements
• Intellectual property rights and assignment clauses
...
• Non-compete clauses (where legally enforceable)`
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1552664730-d307ca884978?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    category: "Contracting",
    detailedContent: `• Independent contractor agreements with clear scope of work
• Deliverable-based contracts with milestones and deadlines
• Payment terms and invoicing procedures
• Intellectual property ownership for work products
• Liability and indemnification clauses
• Termination conditions and dispute resolution
• Compliance with local contractor classification laws`
  },
  {
    id: 3,
    icon: Shield,
    ...
    category: "Compliance",
    detailedContent: `• Review of agreements for compliance with labor laws
• Adherence to minimum wage and working hour regulations
• Compliance with benefits and leave policies
• Data protection and privacy law compliance
• Industry-specific regulatory requirements
• International compliance for remote teams
• Regular updates for changing employment legislation`
  }
];

const benefits = [
  'Protect your business with legally sound agreements',
  'Reduce disputes with clear roles and terms',
  'Ensure compliance with local and international labor laws',
];

export default function EmployeeAgreementsPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<EmployeeAgreementServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: EmployeeAgreementServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    employeeAgreementServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: EmployeeAgreementServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes custom drafting, compliance checks, and legal review to ensure your agreements protect all parties and support business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert employment agreements:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Employee Agreements visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: EmployeeAgreementServiceInfo; 
  index: number; 
  onClick: (info: EmployeeAgreementServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our Employee Agreement Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our employee and contractor agreement services ensure your team relationships are clearly defined, legally protected, and aligned for long-term success.
        </p>
        <Link
          href="/contact"
