...
import { Bookmark, FileText, Shield, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for IP Protection service info
interface IPProtectionServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// IP Protection services data
const ipProtectionServices = [
  {
    id: 1,
    icon: Bookmark,
    ...
    detailedContent: `• Comprehensive trademark search and analysis
• Application preparation and filing
• Monitoring and responding to office actions
• Trademark renewal services
• International trademark registration
• Enforcement and infringement monitoring
• Portfolio management strategies`
  },
  {
    ...
    detailedContent: `• Patentability assessment and prior art search
• Patent drafting and application preparation
• Domestic and international patent filing
...
• Patent infringement analysis and opinions`
  },
  {
    ...
    detailedContent: `• Copyright registration for various works
• Software and digital content protection
• Copyright assignment and licensing
• International copyright protection
• Copyright infringement analysis
• Digital Millennium Copyright Act (DMCA) services
• Creative work portfolio management`
  }
];

const benefits = [
  'Safeguard your brand\'s identity and innovations',
  'Prevent unauthorized use of your intellectual property',
  'Enhance your startup\'s market value with protected IP',
];

export default function IPProtectionPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<IPProtectionServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: IPProtectionServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    ipProtectionServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: IPProtectionServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Safeguard Your Startup's Innovations with IP Protection
        </p>
      </motion.div>
    </section>
  );
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Your startup's intellectual property (IP) is one of its most valuable assets. At Leonard Corporate Solutions, we provide comprehensive IP protection services, including trademark, patent, and copyright registration, to safeguard your innovations in India and worldwide.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes trademark registration, patent filing, and copyright protection to ensure your intellectual property is legally protected and supports business growth.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert IP protection:{" "}
...
const ServiceCard = ({ service, index, onClick }: { 
  service: IPProtectionServiceInfo; 
  index: number; 
  onClick: (info: IPProtectionServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our IP protection services ensure your innovations are legally protected, supporting your business growth and market competitiveness.
        </p>
        <Link
          href="/contact"
