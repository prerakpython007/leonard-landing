...
import { Building, FileText, Settings, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Incorporation service info
interface IncorporationServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
const incorporationServices = [
  {
    ...
    detailedContent: `• Analysis of your business goals and requirements
• Comparison of different legal structures (Proprietorship, LLP, Pvt Ltd)
• Guidance on tax implications for each structure
...
• Industry-specific structure recommendations`
  },
  {
    ...
    detailedContent: `• Document preparation and verification
• Company name reservation and approval
• Registration with Registrar of Companies (ROC)
• Obtaining Certificate of Incorporation
• DIN and DSC application processing
• MOA and AOA drafting and filing
• PAN and TAN registration assistance`
  },
  {
    id: 3,
    icon: Settings,
    ...
    category: "Compliance",
    detailedContent: `• Business bank account setup guidance
• GST registration assistance
• Compliance calendar creation and monitoring
• Initial bookkeeping setup
• Statutory registrations guidance
• Annual compliance requirements briefing
• License and permit acquisition support`
  }
];

const benefits = [
  'Establish a solid legal foundation for growth',
  'Streamline compliance with expert guidance',
  'Choose the right structure to optimize taxes and liability',
];

export default function IncorporationPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<IncorporationServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: IncorporationServiceInfo) => {
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: IncorporationServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      <div className="absolute inset-0">
        <img
          ...
          alt="Incorporation & Structuring hero"
            ...
            >
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach includes structure guidance, incorporation process management, and post-incorporation support to ensure your business starts on a solid legal foundation.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert incorporation services:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Incorporation & Structuring visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: IncorporationServiceInfo; 
  index: number; 
  onClick: (info: IncorporationServiceInfo) => void; 
}) => {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our incorporation services ensure your business starts with the right legal structure, compliance framework, and support system for sustainable growth.
        </p>
        <Link
          href="/contact"
