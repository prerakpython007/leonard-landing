...
import { DollarSign, FileSignature, PieChart, X } from 'lucide-react';
import Link from 'next/link';
import { useState, useCallback, useMemo, type JSX } from 'react';

// Define the interface for Investor Agreement service info
interface InvestorAgreementServiceInfo {
  id: number;
  title: string;
  description: string;
  category: string;
...
// Investor Agreement services data
const investorAgreementServices = [
  {
    id: 1,
    icon: FileSignature,
    title: 'Term Sheets',
    description: 'We draft clear, investor-friendly term sheets outlining key deal terms.',
    ...
    category: "Drafting",
    detailedContent: `• Clear outline of investment terms and conditions
• Valuation and investment amount specifications
• Voting rights and board representation
• Liquidation preferences and anti-dilution provisions
• Investor protections and founder obligations
• Conditions precedent to funding`
  },
  {
    ...
    icon: DollarSign,
    title: 'SAFE Notes',
    description: 'We create SAFE notes to simplify early-stage investments with flexibility.',
    ...
    detailedContent: `• Simple Agreement for Future Equity structuring
• Conversion triggers and valuation caps
• Discount rates for early investors
• Pre-money and post-money valuation considerations
...
• Termination and expiration clauses`
  },
  {
    id: 3,
    icon: PieChart,
    title: 'Equity Agreements',
    description: 'We structure equity deals to align investor and founder interests.',
    ...
    detailedContent: `• Equity allocation and vesting schedules
• Founder stock restrictions and cliffs
...
const benefits = [
  'Attract investors with clear and fair terms',
  'Simplify funding with flexible agreements',
  'Protect your startup while securing investment',
];

export default function InvestorAgreementsPage() {
  const [showPopup, setShowPopup] = useState(false);
  const [selectedInfo, setSelectedInfo] = useState<InvestorAgreementServiceInfo | null>(null);

  const handleExploreClick = useCallback((info: InvestorAgreementServiceInfo) => {
  ...
  const serviceElements = useMemo(() => 
    investorAgreementServices.map((service, index) => (
      <ServiceCard 
        key={service.title} 
        service={service} 
        index={index} 
...
const Popup = ({ selectedInfo, onClose }: { selectedInfo: InvestorAgreementServiceInfo; onClose: () => void }) => {
  return (
    <motion.div
    ...
    <section className="relative h-[70vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
      ...
      >
        ...
        <p className="mt-4 sm:mt-6 max-w-3xl text-base sm:text-lg md:text-xl text-white/90 font-light px-4">
          Attract Investment with Clear, Fair Agreements
        </p>
      </motion.div>
    </section>
  );
              ...
              <div className="bg-white/95 p-6 md:p-8 lg:p-12" style={{ clipPath: "polygon(0 0, calc(100% - 40px) 0, 100% 100%, 0 100%)" }}>
                <div className="space-y-4 md:space-y-6">
                  ...
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Securing investment requires clear and fair agreements that protect both startups and investors. At Leonard Corporate Solutions, we craft investor-friendly term sheets, SAFE notes, and equity agreements to streamline funding rounds and build trust.
                  </p>
                  <p className="text-base md:text-lg leading-relaxed text-[#393E46]">
                    Our comprehensive approach ensures your investment agreements are structured to align interests, protect your business, and facilitate growth while maintaining compliance with relevant regulations.
                  </p>
                  <p className="text-base md:text-lg font-medium text-[#393E46]">
                    📩 For expert investor agreements:{" "}
            ...
            <div className="relative z-10 h-64 md:h-96 lg:h-[520px] w-full lg:w-4/5">
              <div className="absolute inset-0">
                <img 
                  ...
                  alt="Investor Agreements visual" 
...
const ServiceCard = ({ service, index, onClick }: { 
  service: InvestorAgreementServiceInfo; 
  index: number; 
  onClick: (info: InvestorAgreementServiceInfo) => void; 
}) => {
  return (
    <motion.div
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mt-16 md:mt-24 lg:mt-32">
          <motion.h2
            ...
            viewport={{ once: true }}
          >
            Our Investor Agreement Services
          </motion.h2>
    ...
    >
      <div className="relative rounded-xl md:rounded-2xl py-8 md:py-12 backdrop-blur-sm text-center">
        <p className="text-center text-base md:text-lg leading-relaxed text-[#eeeeee] mb-6">
          At Leonard Corporate Solutions, our investor agreement services ensure your funding rounds are structured clearly, legally protected, and aligned for successful growth.
        </p>
        <Link
          href="/contact"
