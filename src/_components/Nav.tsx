        ...
        dropdown: [
          {
            ...
            items: [
              ...
              {
                label: "Global Copyright Filing (as per Berne Convention)",
                href: "/service/international/copyright-berne",
              },
              {
                label: "International Design Protection under Hague System",
                href: "/service/international/design-hague",
              },
            ],
          },
          {
            ...
            items: [
              ...
              {
                label: "Legal Support for Indian Startups Going Global",
                href: "/service/international/startup-global",
              },
              ...
              {
                label: "Cease & Desist + Enforcement in Foreign Jurisdictions",
                href: "/service/international/enforcement",
              },
            ],
          },
        ],
        ...
        dropdown: [
          {
            ...
            items: [
              { label: "Trademark Registration Guide", href: "/blog/Trademark-Registration-Guide" },
              ...
              { label: "Startup IP Strategy", href: "/blog/startupIpStrategy" },
            ],
          },
        ],
      },
  ...
  const renderNavItem = (item: NavItem, position: "left" | "right") => (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        {item.dropdown && <ChevronDown size={16} className="text-gray-500 ml-1" />}
      </div>
      ...
      <AnimatePresence>
        {item.dropdown && openDropdownLabel === item.label && (
          <motion.div
            ...
            className={`absolute ${position === "left" ? "left-0" : "right-0"} top-full pt-3 w-[300px] sm:w-[380px] md:w-[420px] lg:w-[480px] max-w-[calc(100vw-16px)] z-[9999]`}
          >
            <div className="bg-white border border-gray-100 shadow-lg p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {item.dropdown.map((column, idx) => (
                  <div key={idx}>
                    <h3 className="font-semibold text-gray-800 mb-2 uppercase text-xs sm:text-sm">{column.title}</h3>
        ...
        <div className="flex items-center h-20">
          ...
          <div className="hidden md:flex items-center w-full h-full">
            ...
            <div className="flex flex-1 justify-between items-center max-w-[40%]">
              {navigationData.left.map((item) => renderNavItem(item, "left"))}
            </div>
            ...
            <div className="flex items-center h-full py-2 flex-shrink-0 mx-auto">
              <Link href="/" className="h-20 w-auto flex items-center justify-center">
                <AnimatedLogo className="w-20 h-16 relative p-1" />
              </Link>
            </div>

            {/* Right Navigation */}
            <div className="flex flex-1 justify-between items-center max-w-[40%]">
              {navigationData.right.map((item) => renderNavItem(item, "right"))}
            </div>
          </div>
          ...
          <div className="flex md:hidden items-center h-full py-2 ml-auto">
            <Link href="/" className="h-16 w-auto flex items-center">
              <AnimatedLogo className="w-16 h-12 relative p-1" />
            </Link>
          </div>
              ...
              <div className="p-4 h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="space-y-1">
                    {[...navigationData.left, ...navigationData.right].map((item) => (
                              ...
                              <div className="pl-4 pr-2 py-2 space-y-3">
                                {item.dropdown.map((section, idx) => (
                                  <div key={idx} className="space-y-2">
                                    <h3 className="text-sm font-semibold text-gray-800 p-1 uppercase">
                                      {section.title}
                                    </h3>
